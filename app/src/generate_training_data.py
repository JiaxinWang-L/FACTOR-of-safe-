...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--search-mode",
        choices=("fast", "standard", "accurate", "legacy"),
        default="fast",
        help="Slip surface search strategy. Use fast for large synthetic datasets.",
    )
    parser.add_argument("--max-attempt-factor", type=int, default=50, help="Attempt limit multiplier.")
    parser.add_argument("--progress-interval", type=int, default=500, help="Print progress every N valid samples.")
...
def generate_training_data(args: argparse.Namespace) -> pd.DataFrame:
    ...
    progress_interval = max(int(getattr(args, "progress_interval", 500)), 1)
    attempts = 0
    ...
    while len(rows) < args.n_samples and attempts < max_attempts:
        ...
        if len(rows) % progress_interval == 0:
            print(f"generated {len(rows)} / {args.n_samples} valid samples", flush=True)
